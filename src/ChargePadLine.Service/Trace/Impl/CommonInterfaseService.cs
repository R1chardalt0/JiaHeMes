...
namespace ChargePadLine.Service.Trace.Impl
{
    ...
    public class CommonInterfaseService : ICommonInterfaseService
    {
        ...
        public async Task<FSharpResult<ValueTuple, (int, string)>> UploadCheck(RequestUploadCheckParams request)
        {
            ...
            // 5.1. 检查SN格式是否正确
            var result = ValidateSN(request.SN);
            if (result.IsValid)
            {
                Console.WriteLine($"供应商代码: {result.SupplierCode}");
                Console.WriteLine($"生产日期: {result.ProductionDate}");
                Console.WriteLine($"批号: {result.BatchNumber}");
                Console.WriteLine($"零部件品号: {result.PartNumber}");
                Console.WriteLine($"软件版本号: {result.SoftwareVersion}");
                Console.WriteLine($"硬件版本号: {result.HardwareVersion}");
            }
            else
            {
                return FSharpResult<ValueTuple, (int, string)>.NewError((-1, $"错误信息: {result.ErrorMessage}"));
            }
        ...
        public SNAnalysisResult ValidateSN(string sn)
        {
            var result = new SNAnalysisResult();

            if (string.IsNullOrEmpty(sn))
            {
                result.IsValid = false;
                result.ErrorMessage = "SN不能为空";
                return result;
            }

            // 检查总长度：3+6+4+11+2+2 = 28位
            if (sn.Length != 28)
            {
                result.IsValid = false;
                result.ErrorMessage = $"SN长度不正确。期望28位，实际{sn.Length}位";
                return result;
            }
            ...
            {
                // 解析各个字段
                result.SupplierCode = sn.Substring(0, 3);           // 供应商代码 (0-2)
                result.ProductionDate = sn.Substring(3, 6);         // 生产日期 (3-8)
                result.BatchNumber = sn.Substring(9, 4);            // 批号 (9-12)
                result.PartNumber = sn.Substring(13, 11);           // 零部件品号 (13-23)
                result.SoftwareVersion = sn.Substring(24, 2);       // 软件版本号 (24-25)
                result.HardwareVersion = sn.Substring(26, 2);       // 硬件版本号 (26-27)

                // 验证各个字段
                var validationResults = new List<string>();

                // 验证供应商代码（通常为字母数字）
                if (!Regex.IsMatch(result.SupplierCode, @"^[A-Z0-9]{3}$"))
                    validationResults.Add("供应商代码格式不正确");

                // 验证生产日期（应该是数字）
                if (!Regex.IsMatch(result.ProductionDate, @"^\d{6}$"))
                    validationResults.Add("生产日期格式不正确");
                else
                {
                    // 进一步验证日期有效性（可选）
                    if (!IsValidProductionDate(result.ProductionDate))
                        validationResults.Add("生产日期无效");
                }

                // 验证批号（通常为数字）
                if (!Regex.IsMatch(result.BatchNumber, @"^\d{4}$"))
                    validationResults.Add("批号格式不正确");

                // 验证零部件品号（根据实际情况调整规则）
                if (!Regex.IsMatch(result.PartNumber, @"^[A-Z0-9]{11}$"))
                    validationResults.Add("零部件品号格式不正确");

                // 验证版本号（通常为字母数字）
                if (!Regex.IsMatch(result.SoftwareVersion, @"^[A-Z0-9]{2}$"))
                    validationResults.Add("软件版本号格式不正确");

                if (!Regex.IsMatch(result.HardwareVersion, @"^[A-Z0-9]{2}$"))
                    validationResults.Add("硬件版本号格式不正确");
            ...
            {
                result.IsValid = false;
                result.ErrorMessage = $"解析SN时发生错误: {ex.Message}";
            }

            return result;
        }

        private bool IsValidProductionDate(string dateStr)
        {
            // 假设格式为 YYMMDD
            ...
            try
            {
                ...
                // 更精确的日期验证（可选）
                DateTime productionDate = new DateTime(fullYear, month, day);
        ...
        // 生成SN的方法（如果需要）
        public string GenerateSN(string supplierCode, string productionDate,
                               string batchNumber, string partNumber,
            ...
            var lengths = new Dictionary<string, int>
        {
            ...
            {
                if (kvp.Value.Length != lengths[kvp.Key])
                {
                    throw new ArgumentException($"{kvp.Key}长度必须为{lengths[kvp.Key]}位，实际为{kvp.Value.Length}位");
                }
            }

            // 拼接SN
            return $"{supplierCode}{productionDate}{batchNumber}{partNumber}{softwareVersion}{hardwareVersion}";
        }
        /// <summary>
        /// SN追溯DTO
        /// </summary>
