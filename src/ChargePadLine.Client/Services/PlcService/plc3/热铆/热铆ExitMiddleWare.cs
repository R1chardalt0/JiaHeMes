...
namespace ChargePadLine.Client.Services.PlcService.plc3.热铆
{
    public class 热铆ExitMiddleWare : IPlc3Task
    {
        ...
        public 热铆ExitMiddleWare(ILogger<热铆ExitMiddleWare> logger, ILogService logService, 热铆ExitModel exitModel, IOptions<StationConfig> stationconfig, IMesApiService mesApi)
        {
            _logger = logger;
        ...
        public async Task ExecuteOnceAsync(S7NetConnect s7Net, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var exitok = s7Net.ReadBool("DB5010.2.4").Content;//出站OK
                var exitng = s7Net.ReadBool("DB5010.2.5").Content;//出站NG
