...
namespace ChargePadLine.Client.Services.PlcService.plc11.安装支架
{
    public class 安装支架ExitMiddleWare : IPlc11Task
    {
        ...
        public 安装支架ExitMiddleWare(ILogger<安装支架ExitMiddleWare> logger, ILogService logService, 安装支架ExitModel exitModel, IOptions<StationConfig> stationconfig, IMesApiService mesApi)
        {
            _logger = logger;
        ...
        public async Task ExecuteOnceAsync(S7NetConnect s7Net, CancellationToken cancellationToken)
        {
            try
            {
                ...
                if (req && !resp)
                {
                    ...
                    string IsOK = "";
                    var OKRes = s7Net.ReadInt32("DB4024.62").Content;
                    var NGRes = s7Net.ReadInt32("DB4024.66").Content;
