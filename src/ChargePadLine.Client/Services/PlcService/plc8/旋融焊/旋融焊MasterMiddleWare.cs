...
{
    public class 旋融焊MasterMiddleWare : IPlc8Task
    {
        private readonly ILogger<旋融焊ExitMiddleWare> _logger;
        private readonly ILogService _logService;
        private readonly 旋融焊MasterModel _masterModel;
        private readonly StationConfig _stationconfig;
        private readonly IMesApiService _mesApi;
        private const string PlcName = "【旋融焊】";
        private List<TestDataItem> testDatas = new List<TestDataItem>();

        public 旋融焊MasterMiddleWare(ILogger<旋融焊ExitMiddleWare> logger, ILogService logService, IOptions<StationConfig> stationconfig, IMesApiService mesApi, 旋融焊MasterModel masterModel)
        {
            _logger = logger;
        ...
        public async Task ExecuteOnceAsync(ModbusConnect modbus, CancellationToken cancellationToken)
        {
            try
            {
                var req = modbus.ReadBool("3000.0").Content;
