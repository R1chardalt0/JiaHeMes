...
namespace ChargePadLine.Client.Services.PlcService.plc4.后盖超声波焊接
{
    public class 后盖超声波焊接MasterMiddleWare : IPlc4Task
    {
        ...
        public 后盖超声波焊接MasterMiddleWare(ILogger<后盖超声波焊接MasterMiddleWare> logger, ILogService logService, IOptions<StationConfig> stationconfig, IMesApiService mesApi)
        {
            _logger = logger;
