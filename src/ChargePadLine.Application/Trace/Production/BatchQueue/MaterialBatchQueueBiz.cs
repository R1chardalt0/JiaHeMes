    ...
    public class MaterialBatchQueueBiz
    {
        ...
        public async Task<FSharpResult<List<扣料描述>, IErr_批次扣料>> 扣料Async(CmdArg_物料批_扣料 cmdArg)
        {
            var q =
                ...
                from _ in this._repo.SaveChangesAsync().ToOkResult<List<扣料描述>, IErr_批次扣料>(op)
                select op;
            return await q;
        }
        ...
        public async Task<FSharpResult<扣料描述, IErr_批次扣料>> 扣料Async(BatchMaterialQueueItem item)
        {
            var q =
                ...
                from _ in this._repo.SaveChangesAsync().ToOkResult<扣料描述, IErr_批次扣料>(op)
                select op;
            return await q;
        }
        ...
        public async Task<FSharpResult<BatchMaterialQueueItem, IErr>> 调整优先级Async(CmdArg_物料批_调整优先级 cmdArg)
        {
           var q =
               ...
               from _ in this._repo.SaveChangesAsync().ToOkResult<BatchMaterialQueueItem, IErr>(op)
               select op;
           return await q;
        }
