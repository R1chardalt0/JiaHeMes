...
import { getProductTraceInfoList } from '@/services/Api/Trace/ProductTraceInfo';
...
const TableList: React.FC = () => {
    ...
    const handleExport = useCallback(() => {
        ...
        link.setAttribute('download', `产品追溯信息_选中数据_${new Date().toISOString().slice(0, 10)}.csv`);
    ...
    const handleExportAll = useCallback(async () => {
        ...
        try {
            // 获取总条数
            const totalResponse = await getProductTraceInfoList({
                ...currentSearchParams,
                pageSize: 1
            });
            ...
            // 一次性获取所有数据
            const allDataResponse = await getProductTraceInfoList({
                ...currentSearchParams,
                pageSize: total,
                current: 1
            });
            ...
            link.setAttribute('href', url);
            link.setAttribute('download', `产品追溯信息_全部数据_${new Date().toISOString().slice(0, 10)}.csv`);
            link.style.visibility = 'hidden';
        ...
        <PageContainer>
            <ProTable<ProductTraceInfoDto>
                ...
                request={async (
                    ...
                    try {
                        const response = await getProductTraceInfoList(queryParams);
