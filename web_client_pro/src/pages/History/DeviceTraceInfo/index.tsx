...
import { getHistoryEqumentTraceinfoList } from '@/services/Api/Trace/HistoryDeviceOperation';
...
const TableList: React.FC = () => {
  ...
  const handleExport = useCallback(() => {
    ...
    link.setAttribute('download', `历史设备追溯信息_选中数据_${new Date().toISOString().slice(0, 10)}.csv`);
  ...
  const handleExportAll = useCallback(async () => {
    ...
    try {
      // 获取总条数
      const totalResponse = await getHistoryEqumentTraceinfoList({
        ...currentSearchParams,
        pageSize: 1
      });
      ...
      // 一次性获取所有数据
      const allDataResponse = await getHistoryEqumentTraceinfoList({
        ...currentSearchParams,
        pageSize: total,
        current: 1
      });
      ...
      link.setAttribute('href', url);
      link.setAttribute('download', `历史设备追溯信息_全部数据_${new Date().toISOString().slice(0, 10)}.csv`);
      link.style.visibility = 'hidden';
    ...
    <PageContainer>
      <ProTable<EqumentTraceinfoDto>
        ...
        request={async (
          ...
          try {
            const response = await getHistoryEqumentTraceinfoList(queryParams);
